...
function exportMapSVG() {
    ...
    const container = document.getElementById('cy');
    if (!container) { toast('No se encontró el SVG del mapa', 'error'); return; }

    // 1. Guardar la vista actual del usuario
    const savedZoom = x6Graph.zoom();
    const savedTranslate = x6Graph.translate();

    // 2. Forzar fit del contenido completo. X6 hace view-culling: si parte del
    //    grafo está fuera del viewport, esos nodos no están en el DOM. Hacer
    //    zoomToFit garantiza que todo entre y se renderice.
    try { x6Graph.zoomToFit({ padding: 24, maxScale: 1 }); } catch (e) {}

    // 3. Esperar al próximo frame para que el DOM refleje el fit
    requestAnimationFrame(() => {
        try {
            ...
            if (!bbox || !bbox.width || !bbox.height) { toast('El mapa está vacío', 'warning'); return; }
            ...
            // 4. Clonar y resetear transforms y clip-paths que limiten el viewport
            const clone = liveSvg.cloneNode(true);
            clone.querySelectorAll('g').forEach((g) => {
                const cls = g.getAttribute('class') || '';
                if (/x6-graph-svg-(viewport|stage|primer)/.test(cls)) {
            ...
            // Quitar elementos display:none que X6 hubiese marcado
            ...
            // 5. Resolver CSS variables (no se renderizan fuera del browser)
            ...
            console.error('exportMapSVG failed:', err);
            toast('Falló la exportación: ' + err.message, 'error', 5000);
        } finally {
            // 8. Restaurar la vista del usuario
            try {
                x6Graph.zoomTo(savedZoom);
                if (savedTranslate) x6Graph.translate(savedTranslate.tx, savedTranslate.ty);
            } catch (e) {}
        }
    });
}
